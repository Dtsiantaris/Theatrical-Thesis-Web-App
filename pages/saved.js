  ...
  useEffect(() => {
    if (localStorage.getItem("favoriteArtists") !== null) {
      dispatch({
        ...
        value: JSON.parse(localStorage.getItem("favoriteArtists")),
      });
    }
    if (localStorage.getItem("favoriteShows") !== null) {
      dispatch({
        field: "favoriteShows",
        value: JSON.parse(localStorage.getItem("favoriteShows")),
      });
    }
    if (localStorage.getItem("favoriteVenues") !== null) {
      dispatch({
        field: "favoriteVenues",
        value: JSON.parse(localStorage.getItem("favoriteVenues")),
      });
    }
    ...
    <div className="pageWrapper">
      <div className="pageContent">
        <Typography variant="h2" component="h1">
          Αποθηκευμένα <BookmarksIcon />
        </Typography>
        {state.watchlist.ids.length > 0 ||
        state.favoriteArtists.ids.length > 0 ||
        state.favoriteShows.ids.length > 0 ||
        state.favoriteVenues.ids.length > 0 ? (
          <>
            {router.query.showMore ? (
              <Category
                id={state[router.query.showMore].id}
                ids={state[router.query.showMore].ids}
                title={state[router.query.showMore].title}
                path={state[router.query.showMore].path}
              />
            ) : (
              [
                state.watchlist,
          ...
          <Typography varianth="body1" style={{ marginTop: "2em" }}>
            Δεν έχετε προσθέσει κάτι στα αποθηκευμένα σας!
          </Typography>
        )}
      </div>
    </div>
