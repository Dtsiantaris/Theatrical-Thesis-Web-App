...
export const getStaticProps = async ({ params }) => {
  ...
  if (params.query[1]) dateEnd = endOfMonth(dateStart);
  ...
  filteredEvents.forEach((event) => {
    ...
    if (!eventsByVenueMap.has(event.VenueID)) {
      eventsByVenueMap.set(event.VenueID, {
        value: 1,
        shows: [event.ProductionID],
      });
    } else {
      eventsByVenueMap.get(event.VenueID).value++;
      if (
        !eventsByVenueMap.get(event.VenueID).shows.includes(event.ProductionID)
      ) {
    ...
    if (!showsByMonthMap.has(eventDate.getMonth())) {
      ...
      if (
        !showsByMonthMap.get(eventDate.getMonth()).includes(event.ProductionID)
      ) {
  ...
  eventsByShow = await Promise.all(
    eventsByShow.map(async (show) => {
  ...
  eventsByVenue = await Promise.all(
    eventsByVenue.map(async (venue) => {
  ...
  if (eventsByVenue[0]) {
    eventsByVenue[0].eventsByShow = await Promise.all(
      eventsByVenue[0].eventsByShow.map(async (show) => {
  ...
  priceByShow = await Promise.all(
    priceByShow.map(async (show) => {
      const production = await mainFetcher(`/productions/${show.id}`);
...
const StatsPage = ({
  ...
  const lastDayMonth = useMemo(() => {
    if (router.query.query)
      return endOfMonth(
        new Date(router.query.query[0] || 2021, router.query.query[1] || 0)
      );
  }, [router.query.query]);

  const handleDateChange = (date) => {
    setSelectedDate(startOfMonth(date));
    if (mode === "year") router.push(`/stats/${date.getFullYear()}`);
    else router.push(`/stats/${date.getFullYear()}/${date.getMonth()}`);
  };
  ...
  const handleBarClick = async (payload) => {
    if (payload) {
      ...
      if (!eventsByVenue[payload.activeTooltipIndex].eventsByShow[0].name) {
        const showIds = eventsByVenue[
          payload.activeTooltipIndex
        ].eventsByShow.map((show) => show.id);
        const names = await internalFetcher("/api/getShowNameById", showIds);
        eventsByVenue[payload.activeTooltipIndex].eventsByShow.forEach(
          (show) => {
  ...
  useEffect(() => {
    if (router.query.query)
      setSelectedDate(
        new Date(router.query.query[0], router.query.query[1] || 0)
      );
  }, [router.query.query]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const shows = await internalFetcher("/api/getShowsByDate", {
      ...
      <div className="pageWrapper" style={{ overflow: "hidden" }}>
        <div className="pageContent">
          <Typography variant="h2" component="h1">
            Στατιστικά {router.query.query[1] && months[router.query.query[1]]}{" "}
          ...
          <div className={classes.picker}>
            ...
            <ThemeProvider
              theme={() => DatePickerTheme(theme.palette.secondary.main)}
            >
              <DatePicker
          ...
          {eventsByDate.length > 0 ? (
            <>
              <div style={{ marginTop: 50 }}>
                ...
                <div className={classes.calendarContainer}>
                  ...
                  <TimeRange
                    ...
                    monthLegend={(year, month, date) =>
                      date.toLocaleDateString("el", { month: "short" })
                    }
                ...
                <div>
                  <Typography variant="h4" component="h3">
                    {selectedDate.toLocaleDateString("el", {
                      day: "numeric",
                      weekday: "long",
                      month: "long",
                      year: "numeric",
                    ...
                    <div className={classes.listContainer}>
                      <ul
                        className={`${classes.list} ${
                          listVisible ? "" : classes.listHidden
                        }`}
                      >
                        {showsByDate &&
                      ...
                      <IconButton
                        ...
                        className={`${classes.expandButton} ${
                          listVisible ? classes.collapseButton : ""
                        }`}
              ...
              <div className={classes.flexChartContainer}>
                ...
                <div className={classes.chartContainer}>
                  <Typography variant="h3" component="h2">
                    Παραγωγές Με Τις Περισσότερες Παραστάσεις
                  </Typography>
                ...
                <div className={classes.chartContainer}>
                  <Typography variant="h3" component="h2">
                    Θέατρα Με Τις Περισσότερες Παραστάσεις
                  </Typography>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                ...
                <div className={classes.chartContainer}>
                  <Typography
                    variant="h3"
                    component="h2"
                  >{`${eventsByVenue[activeIndex].name} - Παραστάσεις Ανά Παραγωγή`}</Typography>
                  {loading ? (
                    <div className={classes.loadingContainer}>
                      <CircularProgress color="secondary" />
                    </div>
            ...
            <>
              {router.query.query[0] && (
                <Typography>
                  Δεν υπάρχουν στατιστικά για την συγκεκριμένη περίοδο!
                </Typography>
              )}
            </>
          )}
