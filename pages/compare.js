...
export const getServerSideProps = async ({ query }) => {
  ...
  const firstDateLabel =
    query.mode === "year"
      ? firstDateStart.getFullYear()
      : `${months[firstDateStart.getMonth()]} ${firstDateStart.getFullYear()}`;
  const secondDateLabel =
    query.mode === "year"
      ? secondDateStart.getFullYear()
      : `${
          months[secondDateStart.getMonth()]
        } ${secondDateStart.getFullYear()}`;
  ...
  const size =
    query.mode === "year"
      ? Math.max(getDaysInYear(firstDateStart), getDaysInYear(secondDateStart))
      : Math.max(
          getDaysInMonth(firstDateStart),
          getDaysInMonth(secondDateStart)
        );
  ...
  const getEventsByDate = (dateStart, dateEnd) => {
    ...
    filteredEvents.forEach((event) => {
      const eventDate = new Date(event.DateEvent);
      const day =
        query.mode === "year" ? getDayOfYear(eventDate) : eventDate.getDate();
  ...
  const getNumberOfShowsByMonth = (dateStart, dateEnd) => {
    ...
    filteredEvents.forEach((event) => {
      ...
      if (!showsByMonthMap.has(eventDate.getMonth())) {
        ...
        if (
          !showsByMonthMap
            .get(eventDate.getMonth())
            .includes(event.ProductionID)
        ) {
  ...
  if (query.stat === "showsByMonth") {
    ...
    const showsByMonth2 = getNumberOfShowsByMonth(
      secondDateStart,
      secondDateEnd
    );
  ...
  if (query.stat === "avgPriceByDate") {
    ...
    const avgPriceByDate2 = getTicketAveragePrice(
      secondDateStart,
      secondDateEnd
    );
  ...
  if (query.stat === "eventsByMonth") {
    const eventsByMonth1 = getNumberOfEventsByMonth(
      firstDateStart,
      firstDateEnd
    );
    const eventsByMonth2 = getNumberOfEventsByMonth(
      secondDateStart,
      secondDateEnd
    );
...
const ComparePage = ({ stat, labels, statTitle }) => {
  ...
  const [statValue, setStatValue] = useState(
    router.query.stat || "eventsByDate"
  );

  const [firstDate, setFirstDate] = useState(
    new Date(router.query.date1 || "2021")
  );
  const [secondDate, setSecondDate] = useState(
    new Date(router.query.date2 || "2022")
  );
  ...
  const handleSubmit = (event) => {
    ...
    if (
      mode === "month" &&
      (initialStat === "eventsByMonth" || initialStat === "showsByMonth")
    ) {
    ...
    if (date1 !== date2) {
      router.push(
        `/compare?date1=${date1}&date2=${date2}&mode=${mode}&stat=${initialStat}`
      );
    }
  };
          ...
          {router.query.date1 && router.query.date2 && (
            <>
              <div style={{ marginTop: 100 }}>
                ...
                >
                  <MenuItem value="eventsByDate">
                    Αθροιστικός Αριθμός Παραστάσεων ανά Ημέρα
                  </MenuItem>
                  <MenuItem value="totalEvents">Αριθμός Παραστάσεων</MenuItem>
                  {router.query.mode === "year" && (
                    <MenuItem value="eventsByMonth">
                      Παραστάσεις ανά Μήνα
                    </MenuItem>
                  )}
                  <MenuItem value="numberOfShows">Αριθμός Παραγωγών</MenuItem>
                  {router.query.mode === "year" && (
                    <MenuItem value="showsByMonth">Παραγωγές ανά Μήνα</MenuItem>
                  )}
                  <MenuItem value="avgPriceByDate">
                    Μέσος Όρος Τιμών Εισητηρίων ανά Ημέρα
                  </MenuItem>
                </Select>
              </div>
              ...
              <div className={classes.graphContainer}>
                ...
                {(router.query.stat === "eventsByDate" ||
                  router.query.stat === "avgPriceByDate") && (
                ...
                {(router.query.stat === "totalEvents" ||
                  router.query.stat === "numberOfShows") && (
                  <ResponsiveContainer width="100%" height={400}>
                    <PieChart>
                      <Pie data={stat} dataKey="value">
                        <Cell
                          fill={theme.palette.secondary.main}
                ...
                {(router.query.stat === "showsByMonth" ||
                  router.query.stat === "eventsByMonth") && (
                  <ResponsiveContainer width="100%" height={400}>
                    <BarChart
                      data={stat}
                      margin={{
